...
function colorEdges(edges) {
    ...
    // Initialize a set to keep track of assigned colors for adjacent edges
    ...
    edges.forEach((edge, index) => {
        ...
        // Check adjacent edges for used colors
        edges.forEach((adjEdge) => {
            if (
                (adjEdge.source === source || adjEdge.target === source) &&
                (adjEdge.source !== target && adjEdge.target !== target)
            ) {
                if (edgeColors[adjEdge.id] !== undefined) {
                    usedColors.add(edgeColors[adjEdge.id]);
                }
            }
        });

        // Assign the first available color from the color palette
        for (let color of colorPalette) {
            if (!usedColors.has(color)) {
                edgeColors[index] = color; // Assign color to the current edge
                break; // Break once a color is found
            }
        }
    });
    ...
    if (uniqueEdgeColorsCount === maxDegree) {
        document.getElementById('colorInfo').textContent = `Optimal edge coloring found! Number of colors used: ${uniqueEdgeColorsCount}`;
    } else if (uniqueEdgeColorsCount === maxDegree + 1) {
        document.getElementById('colorInfo').textContent = `Edge coloring within upper bound. Number of colors used: ${uniqueEdgeColorsCount}`;
    } else {
        document.getElementById('colorInfo').textContent = `Edge coloring exceeds expected bounds. Number of colors used: ${uniqueEdgeColorsCount}`;
    }
